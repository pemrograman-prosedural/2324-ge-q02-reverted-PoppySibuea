...
{
    if (_student == NULL || _dorm == NULL || old_dorm == NULL)
    {
        printf("Error: Invalid student or dorm pointers.\n");
        return;
    }

    if (_dorm->residents_num >= _dorm->capacity)
    {
        printf("Error: Dorm is full.\n");
        return;
    }

    if (_dorm->gender != _student->gender)
    {
        printf("Error: Dorm gender does not match student gender.\n");
        return;
    }
